...
# Version: v0.001
# Notes: Initial version with players, battles, clans, quests, achievements, market, events.
...
def add_player(username, clan):
    conn = sqlite3.connect("game.db"); c = conn.cursor()
    c.execute("""
        INSERT OR IGNORE INTO players
        (username, energy, points, level, followers, items, last_login, wins, losses, clan)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (username, 10, 0, 1, 0, json.dumps([]), time.time(), 0, 0, clan))
...
def update_player(username, energy, points, level, followers, items, wins=None, losses=None):
    ...
    params = [energy, points, level, followers, json.dumps(items), time.time()]
    if wins is not None:
